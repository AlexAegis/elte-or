...
@Command(name = "change", sortOptions = false, header = {"", ""}, mixinStandardHelpOptions = true,
		descriptionHeading = "@|bold %nDescription|@:%n",
		description = {"", "Changes the playing music's tempo and or transposition",},
...
public class ChangeCommand implements Runnable {
	...
	@CommandLine.Parameters(paramLabel = "<no>", index = "0", arity = "0",
	...
	@Override
	public void run() {
		new Change(parent.getClient().getConnection(), no, tempo, transpose).doFinally(parent.getOut()::flush)
				.map(s -> s.substring(4)).blockingSubscribe(next -> parent.getOut().println("Change result: " + next),
