...
@Command(name = "server",
	...
	description = {"", "Server application for MusicBox",},
...
public class MusicBox implements Runnable {
	...
	public static final Song SONG_TEST1 = new Song("test1", Arrays
		.asList("C 4 E 4 C 4 E 4 G 8 G 8 REP 6;1 C/1 4 B 4 A 4 G 4 F 8 A 8 G 4 F 4 E 4 D 4 C 8 C 8".split(" ")));
	public static final Song SONG_TEST2 = new Song("test2", Arrays.asList(
		("D 1 D 3 D/1 1 D/1 3 C/1 1 C/1 3 C/1 2 C/1 2 D/1 1 D/1 3 C/1 1 Bb 3 A 4 A 2 R 2 REP 15;1 " +
			...
			.split(" ")));
	public static final Song SONG_MEGALOVANIA_MAIN_BASE = new Song("megalovania", Arrays.asList(
		("D 1 D 1 D/1 1 R 1 A 1 R 2 G# 1 R 1 G 1 R 1 F 2 D 1 F 1 G 1 " +
			...
			.split(" ")));
	public static final Song SONG_MEGALOVANIA_MAIN_RAISED = new Song("megalovania_raised", Arrays.asList(
		("R 64 D/1 1 D/1 1 D/2 1 R 1 A/1 1 R 2 G#/1 1 R 1 G/1 1 R 1 F/1 2 D/1 1 F/1 1 G/1 1 " +
			...
			.split(" ")));
	public static final Song SONG_MEGALOVANIA_BASE_BASS = new Song("megalovania_bass", Arrays.asList(
		("R 64 R 32 R 24 R 2 D#/-1 2 D#/-1 2 R 2 G/-2 2 E/-2 2 G/-2 2 E/-2 2 REP 4;6 " +
			...
			.split(" ")));
	public static final Song SONG_MEGALOVANIA_BASE_UPPER = new Song("megalovania_base_upper", Arrays
		.asList("R 64 D 4 D 4 D 4 D 4 C 4 C 4 C 4 C 4 B 4 B 4 B 4 B 4 A# 4 A# 4 C 4 C 4 REP 16;3".split(" ")));
	public static final Song SONG_MEGALOVANIA_BASE_LOWER = new Song("megalovania_base_lower", Arrays.asList(
	...
	public List<Integer> allPlayingByConnection(Connection connection) {
		return playing.entrySet().stream().filter(e -> e.getValue().getX().equals(connection)).map(Map.Entry::getKey)
			.collect(Collectors.toList());
	}
	...
	public void run() {
		...
		try (var server = new ServerSocket(port)) {
			Flowable.fromCallable(() -> new Connection(this, server))
				...
				.flatMap(r -> r.getListener()
					.toFlowable(BackpressureStrategy.MISSING)
					.compose(FlowableTransformers.bufferUntil(new Predicate<>() {
						private int remaining = 0;
						@Override
						public boolean test(String next) {
							ActionType.getActionByName(next.split(" ")[0])
								.ifPresent(actionType -> remaining = actionType.getAdditionalLines());
							return --remaining < 0;
						}
					}))
					.withLatestFrom(Flowable.just(r), Pair::new))
				.flatMap(pair -> ActionType.construct(Observable.just(pair.getY()), pair.getX())
					.toFlowable(BackpressureStrategy.MISSING))
				.sequential()
